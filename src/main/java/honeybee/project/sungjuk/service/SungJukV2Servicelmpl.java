...
public class SungJukV2Servicelmpl implements SungJukV1cService {
        private Scanner sc = null;
        private List<SungJukVO> sjs = null;

        public SungJukV2Servicelmpl() {
            ...
            String name = sc.next();
                ...
                for (SungJukVO sj : sjs) {
                    if (sj.getName().equals(name)) {
                        sjs.remove(sj);  // 대상을 list에서 삭제(객체와 값으로 삭제가능)
                        System.out.println("\n삭제되었습니다\n");
                        break;
                    }
                }
